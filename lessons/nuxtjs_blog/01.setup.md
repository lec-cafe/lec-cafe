...
「Nuxt.jsで始めるWebサイト制作」を確認してみましょう。
:::
...
`assets/common.css` を作成して、以下のCSSの内容を作成しておきましょう。

https://github.com/BlackrockDigital/startbootstrap-clean-blog/blob/master/css/clean-blog.css

:::tip  
上記cssはここに記述すると長くなりすぎる為、記述しませんが。
そのままコピーアンドペーストするとエラーがでる箇所があります。
エラーが出た場合は検討箇所を削除するだけで解決するので試してみてください。
:::

:::tip2  
cssはlinkセクションではなく、cssセクションに保存するように注意してください。
:::
...
:::tip  
このサイトのヘッダーメニューで使用する画像は  
https://github.com/BlackrockDigital/startbootstrap-clean-blog/tree/master/img
に保存されています。
:::
...
`http://localhost:3000/` で一覧画面が、
`http://localhost:3000/post` で詳細画面が表示できれば完成です。
