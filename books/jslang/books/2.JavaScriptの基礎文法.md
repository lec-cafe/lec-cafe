...
結果の切り捨て、切り上げ、四捨五入などを行いたいときには、 `Math.ceil` `Math.floor` `Math.round` などが利用可能です。
...
呼び出す際には関数の変数名を利用して、 `functionName()` のように後ろに `(...)`  をつけて呼び出し、`(...)` の中で引数を渡すことが可能です。
...
`(...) ? ... : ...` の形式で利用され、最初の `...` で条件式を定義し、条件式が true の場合、false の場合に、それぞれ `...` を返却します。条件式の `(  )` は省略することも可能です。 
...
for 文は繰り返しの構文として用いられます。`for(...; ...; ...)` の形式で、順に初期値、継続条件、ループ毎に実行する処理、を指定します。以下のコードでは、 `$i` 変数を定義し、初期値 `0` で、`$i<5` の間ループを実行、ループの度に `$i` を 1づつ加算する、という処理を行っています。
