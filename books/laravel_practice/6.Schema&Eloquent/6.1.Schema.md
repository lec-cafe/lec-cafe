...
インデックスは、データベース内の検索処理を高速化するために作成される、テーブルの索引データです。

### ユニークインデックスの管理

ユニークインデックスは、指定したカラムのデータがテーブル内で一意になるよう指定するインデックスです。
ユニークインデックスを指定した列のデータは、テーブル内で重複する事ができません。

ユニークインデックスの指定は、列定義の末尾に`unique`を呼び出すことで追加出来ます。
